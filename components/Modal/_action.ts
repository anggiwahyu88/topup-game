...
export const add = async (state: any, formData: FormData) => {
    ...
    const { error } = await uploadGame({ image, name, developer, descripsion, name_provider, description_instructions, check_id, status: JSON.parse(`${status}`), server_list, zone_id: JSON.parse(`${zone_id}`) })
    if (error) {
        return {
            valid: false, errors: {
                user: error.message
