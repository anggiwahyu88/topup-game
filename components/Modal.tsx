...
const Modal = ({ handleClose, onModal, action, name }: { handleClose: () => void, onModal: boolean, action: () => void, name: string }) => {
    return (
        <>
            {onModal ?
                <div id="popup-modal" tabIndex={- 1} className={`overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 max-h-full`} style={{ background: "rgba(0, 0, 0, 0.4)" }} >
                    <div className="relative p-4 w-full max-w-md max-h-full left-[50%] top-[50%]" style={{ transform: "translate(-50%, -50%)" }}>
                        <div className="relative rounded-lg shadow bg-gray-700">
                            <button type="button" className="absolute top-3 end-2.5 text-gray-400 bg-transparent rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center hover:bg-gray-600 hover:text-white" onClick={handleClose}>
